...
/*
    Bah. This is a copy of the original LinkedBlockingQueue with two trivial changes - the takeLock
    and putLock have been made public. This is because unstashAll() is ActorRef has race conditions with
    try to get all the stashed messages in correct order at the beginning of the mailbox Q - there is always the chance
    messages could be taken off the Q. Also since we 'freeze' the old Q and then copy it back with an addAll there
    is always the chance some news messages could have come in and so would be sandwiched between the old messages
    and the unstashed one. So we take the lock for both ends of the q, do our juggling and then free things up.

    unstashAll is not on any major paths, so this temporary seizing of the locks will not have any major performance
    impacts ...
 */
/**
...
public class DustLinkedBlockingQueue<E> extends AbstractQueue<E>
    ...
    /**
     * Creates a {@code DustLinkedBlockingQueue} with a capacity of
    ...
    /**
     * Creates a {@code DustLinkedBlockingQueue} with the given (fixed) capacity.
     *
     ...
     */
    public DustLinkedBlockingQueue(Collection<? extends E> c) {
        this(Integer.MAX_VALUE);
        final ReentrantLock putLock = this.putLock;
