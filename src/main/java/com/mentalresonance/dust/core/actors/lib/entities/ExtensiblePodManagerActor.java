 ...
 *
 * Note - this class allows extensions to define their own state as well as the state pod management needs
 *
 *  @author alanl
 */
@Slf4j
public class ExtensiblePodManagerActor extends PersistentActor {
    /**
     * Common Props for children
     */
    protected final Props childProps;

    protected PodManagerState state;
    ...
    public static Props props(Props childProps) {
        return Props.create(ExtensiblePodManagerActor.class, childProps, null, true, List.of());
    }
    ...
    public static Props props(Props childProps, ActorRef podDeadLetterRef) {
        return Props.create(ExtensiblePodManagerActor.class, childProps, podDeadLetterRef, true, List.of());
    }
    ...
    public static Props props(Props childProps, ActorRef podDeadLetterRef, boolean notifyCreatedChild) {
        return Props.create(ExtensiblePodManagerActor.class, childProps, podDeadLetterRef, notifyCreatedChild, List.of());
    }
     ...
     */
    public ExtensiblePodManagerActor(Props childProps, ActorRef podDeadLetterRef, boolean notifyCreatedChild, List<Class<?>> acceptedMessages) {
     ...
     * @param podDeadLetterRef ref to {@link PodDeadLetterActor}
     */
    public ExtensiblePodManagerActor(Props childProps, ActorRef podDeadLetterRef) {
        this.childProps = childProps;
        this.podDeadLetterRef = podDeadLetterRef;
    ...
    protected ActorBehavior recoveryBehavior() {
        return message -> {
            if (Objects.requireNonNull(message) instanceof SnapshotMsg(Serializable snapshot)) {
                state = (PodManagerState) snapshot;
                for (String name : state.getChildren()) {
    ...
    public interface PodManagerState extends Serializable {
